...
void read_header(ifstream &csv_file, int &field_ip, int &field_date, int &field_time, int &field_cik, int &field_accession, int &field_extention)  //The header read function reads the first line of the saves the position of the interesting data fields
{
	string tmp_string;
...
vector<string> read_line(ifstream &csv_file, int &last_field,int &field_ip, int &field_date, int &field_time)  //The read line function reads subsequent lines of the file and pushes the required fields into the vector
{
	string tmp_string;
	...
	while(first_line.good())
	{
		...
		if(counter==last_field)  //Stop if all required fields are accessed
		{
			...
			else if(field_ip<field_time&&field_date>field_time) swap(result[1],result[2]);  //For the less common unordered fields, vector fields are exchanged. !!!Slower processing. If I had more time I would determine the case only once in the header function. But then a global variable or an extra parameter are required
...
int main(int argc, char* argv[]) //argv holds the input and output files
{
	...
	{	
		...
		{
			vec = read_line(csv_file, last_field, field_ip, field_date, field_time);  //Read line and return result vector
	...
	vector<string> clear_LL;   //Vector for residual List entries (Sorting another Double Linked List is probably more efficient. Better solution for a very long list, but not tested because of lack of time)
        vector<int> start_time;   //Use vector to find timing order
        while(tail!=NULL)
        {
        	out2 = dllist.extract_residual_sessions(head,tail,ip_to_delete,time_of_arrival); //Get output string from tail node
        	if(out2!="")
                {
			clear_LL.push_back(out2);  //Add output string to sort vector
			start_time.push_back(time_of_arrival);  //Keep track of the start time
                }
        }
	while(clear_LL.size()!=0)  //Remove all elements from vector
	{
		int min_time=INT_MAX;  //Start with largest possible number  
		int first=-1;
		for(int i=0; i<start_time.size(); i++)
		{
			if(start_time[i]<min_time)
                        {
				min_time=start_time[i];  //Determine earliest request
				first=i;  //Position in vector
                        }
		}
		output_file<<clear_LL[first]<<endl;  //Write out the first occurrence 
		clear_LL.erase(clear_LL.begin()+first);  //Remove string from vector
		start_time.erase(start_time.begin()+first);  //Remove time tag
	}
